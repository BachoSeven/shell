...
echo "\"$(dconf read ${KEYS_GNOME_WM}/switch-to-workspace-down)\"" >> restore_shortcuts
dconf write ${KEYS_GNOME_WM}/switch-to-workspace-down "['<Primary><Super>Down','<Primary><Super>${down}']"
# Move to workspace above
echo "\"$(dconf read ${KEYS_GNOME_WM}/switch-to-workspace-up)\"" >> restore_shortcuts
dconf write ${KEYS_GNOME_WM}/switch-to-workspace-up "['<Primary><Super>Up','<Primary><Super>${up}']"

# Disable tiling to left / right of screen
echo "\"$(dconf read ${KEYS_MUTTER}/toggle-tiled-left)\"" >> restore_shortcuts
dconf write ${KEYS_MUTTER}/toggle-tiled-left "@as []"
echo "\"$(dconf read ${KEYS_MUTTER}/toggle-tiled-right)\"" >> restore_shortcuts
dconf write ${KEYS_MUTTER}/toggle-tiled-right "@as []"

# Toggle maximization state
echo "\"$(dconf read ${KEYS_GNOME_WM}/toggle-maximized)\"" >> restore_shortcuts
dconf write ${KEYS_GNOME_WM}/toggle-maximized "['<Super>m']"
# Lock screen
echo "\"$(dconf read ${KEYS_MEDIA}/screensaver)\"" >> restore_shortcuts
dconf write ${KEYS_MEDIA}/screensaver "['<Super>Escape']"
# Home folder
echo "\"$(dconf read ${KEYS_MEDIA}/home)\"" >> restore_shortcuts
dconf write ${KEYS_MEDIA}/home "['<Super>f']"
# Launch email client
echo "\"$(dconf read ${KEYS_MEDIA}/email)\"" >> restore_shortcuts
dconf write ${KEYS_MEDIA}/email "['<Super>e']"
# Launch web browser
echo "\"$(dconf read ${KEYS_MEDIA}/www)\"" >> restore_shortcuts
dconf write ${KEYS_MEDIA}/www "['<Super>b']"
# Rotate Video Lock
echo "\"$(dconf read ${KEYS_MEDIA}/rotate-video-lock-static)\"" >> restore_shortcuts
dconf write ${KEYS_MEDIA}/rotate-video-lock-static "@as []"

# Close Window
echo "\"$(dconf read ${KEYS_GNOME_WM}/close)\"" >> restore_shortcuts
